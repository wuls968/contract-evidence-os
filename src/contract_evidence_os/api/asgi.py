...
from contract_evidence_os.api.console_routes import (
    build_console_auth_router,
    build_console_config_router,
    build_console_events_router,
    build_console_spa_router,
    build_console_ui_router,
    build_console_usage_router,
)
from contract_evidence_os.api.operator import OperatorAPI
from contract_evidence_os.api.operator_routes import build_legacy_proxy_router, build_operator_router
...
class RemoteOperatorController:
    ...
    def dispatch_get(self, path: list[str], query: dict[str, list[str]]) -> Any:
        ...
        if path == ["strategy", "overview"]:
            scope_key = query.get("scope_key", [None])[0]
            if not scope_key:
                raise KeyError("strategy/overview")
            return self.api.strategy_state(scope_key)
        ...
        if len(path) == 3 and path[0] == "tasks" and path[2] == "collaboration":
            return self.api.task_collaboration(path[1])
        if len(path) == 3 and path[0] == "tasks" and path[2] == "leases":
            return {"items": self.api.task_collaboration(path[1])["active_leases"]}
        if len(path) == 3 and path[0] == "tasks" and path[2] == "branches":
            return {"items": self.api.task_collaboration(path[1])["branches"]}
        ...
        if len(path) == 4 and path[0] == "tasks" and path[2] == "memory" and path[3] == "scopes":
            return self.api.memory_scope_state(path[1])
    ...
    def dispatch_post(self, path: list[str], body: dict[str, Any]) -> Any:
        ...
        if path == ["strategy", "feedback"]:
            return self.api.record_strategy_feedback(
                scope_key=str(body.get("scope_key", "")),
                actor=str(body.get("actor", "remote-operator")),
                strategy_kind=str(body.get("strategy_kind", "summarization_policy")),
                signal_kind=str(body.get("signal_kind", "operator_feedback")),
                metrics=dict(body.get("metrics", {})),
                evidence_refs=[str(item) for item in body.get("evidence_refs", [])],
            ).to_dict()
        if path == ["strategy", "candidates"]:
            return self.api.propose_strategy_candidate(
                scope_key=str(body.get("scope_key", "")),
                actor=str(body.get("actor", "remote-operator")),
                strategy_kind=str(body.get("strategy_kind", "summarization_policy")),
                target_component=str(body.get("target_component", "")),
                hypothesis=str(body.get("hypothesis", "")),
                supporting_signal_ids=[str(item) for item in body.get("supporting_signal_ids", [])],
            ).to_dict()
        if len(path) == 4 and path[0] == "strategy" and path[1] == "candidates" and path[3] == "evaluate":
            return self.api.evaluate_strategy_candidate(
                path[2],
                regression_failures=None if body.get("regression_failures") is None else int(body.get("regression_failures")),
                gain=None if body.get("gain") is None else float(body.get("gain")),
            ).to_dict()
        if len(path) == 4 and path[0] == "strategy" and path[1] == "candidates" and path[3] == "canary":
            return self.api.run_strategy_canary(
                path[2],
                actor=str(body.get("actor", "remote-operator")),
        ...
        if len(path) == 4 and path[0] == "strategy" and path[1] == "candidates" and path[3] == "promote":
            return self.api.promote_strategy_candidate(
                path[2],
                actor=str(body.get("actor", "remote-operator")),
                reason=str(body.get("reason", "remote strategy promotion requested")),
            ).to_dict()
        ...
        if len(path) == 3 and path[0] == "tasks" and path[2] == "collaboration":
            return self.api.configure_task_collaboration(
                task_id=path[1],
                owner=str(body.get("owner", "remote-operator")),
                reviewer=str(body.get("reviewer", "")),
                operators=[str(item) for item in body.get("operators", [])],
                watchers=[str(item) for item in body.get("watchers", [])],
                approval_assignee=str(body.get("approval_assignee", "")),
            ).to_dict()
        if len(path) == 3 and path[0] == "tasks" and path[2] == "leases":
            return self.api.acquire_task_lease(
                task_id=path[1],
                actor=str(body.get("actor", "remote-operator")),
                lease_kind=str(body.get("lease_kind", "owner")),
                phase=str(body.get("phase", "execution")),
                ttl_seconds=int(body.get("ttl_seconds", 900)),
            ).to_dict()
        if len(path) == 3 and path[0] == "tasks" and path[2] == "branches":
            return self.api.create_task_branch(
                task_id=path[1],
                actor=str(body.get("actor", "remote-operator")),
                branch_kind=str(body.get("branch_kind", "research")),
                title=str(body.get("title", "Task branch")),
                parent_branch_id=str(body.get("parent_branch_id", "")),
            ).to_dict()
        if len(path) == 3 and path[0] == "tasks" and path[2] == "handoff":
            return self.api.open_handoff_window(
                task_id=path[1],
                from_actor=str(body.get("from_actor", "remote-operator")),
                to_actor=str(body.get("to_actor", "")),
        ...
        if len(path) == 4 and path[0] == "tasks" and path[2] == "memory" and path[3] == "scopes":
            return self.api.record_scoped_memory(
                task_id=path[1],
                actor=str(body.get("actor", "remote-operator")),
                owner_user_id=str(body.get("owner_user_id", body.get("actor", "remote-operator"))),
                audience_scope=str(body.get("audience_scope", "personal_private")),
                memory_kind=str(body.get("memory_kind", "note")),
                summary=str(body.get("summary", "")),
                content=dict(body.get("content", {})),
                evidence_refs=[str(item) for item in body.get("evidence_refs", [])],
                privacy_level=str(body.get("privacy_level", "medium")),
                contradiction_risk=float(body.get("contradiction_risk", 0.0)),
            ).to_dict()
        if len(path) == 5 and path[0] == "tasks" and path[2] == "memory" and path[3] == "scopes" and path[4] == "promote":
            return self.api.promote_scoped_memory(
                record_id=str(body.get("record_id", "")),
                actor=str(body.get("actor", "remote-operator")),
                target_scope=str(body.get("target_scope", "task_shared")),
                reason=str(body.get("reason", "remote scoped memory promotion requested")),
            ).to_dict()
        if len(path) == 5 and path[0] == "tasks" and path[2] == "memory" and path[3] == "scopes" and path[4] == "summary":
            return self.api.generate_scope_summary(
                task_id=path[1],
                actor=str(body.get("actor", "remote-operator")),
                summary_kind=str(body.get("summary_kind", "handoff_summary")),
            ).to_dict()
    ...
    *,
    ...
    app.include_router(
        build_operator_router(
            ...
            proxy_error_response=proxy_error_response,
        )
    )
    app.include_router(
        build_console_spa_router(
